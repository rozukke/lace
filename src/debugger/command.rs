...
impl TryFrom<&str> for Command {
    ...
    fn try_from(line: &str) -> std::result::Result<Self, Self::Error> {
        let mut iter = iter::CommandIter::from(line);
...
mod iter {
    use super::{Error, Label, Location, MemoryLocation, Result};
    use crate::symbol::Register;
    ...
    impl<'a> CommandIter<'a> {
        ...
        fn next_argument(&mut self) -> Result<Option<Argument>> {
            ...
            if let Some(integer) = self.next_token_integer(true)? {
                return Ok(Some(Argument::Integer(integer)));
            }
            if let Some(label) = self.next_token_label()? {
        ...
        fn next_token_integer(&mut self, allow_sign: bool) -> Result<Option<i32>> {
            self.reset_head();
            // Don't skip whitespace
            ...
            let Some((radix, has_leading_zeros, prefix_is_symbol)) = self.next_integer_prefix()?
            else {
        ...
        fn next_token_label(&mut self) -> Result<Option<Label>> {
            self.reset_head();
            // Don't skip whitespace
            ...
            let offset = match self.peek() {
                Some('+') => {
                    self.next();
                    self.base = self.head;
                    let Some(offset) = self.next_token_integer(false)? else {
    ...
    mod tests {
        ...
        fn next_integer_works() {
            macro_rules! expect_integer { ( $allow_sign:expr, $($x:tt)* ) => {
                expect_tokens!(next_token_integer($allow_sign), $($x)*);
            }}
        ...
        fn next_label_works() {
            macro_rules! expect_label { ( $($x:tt)* ) => {
                expect_tokens!(next_token_label(), $($x)*);
            }}
